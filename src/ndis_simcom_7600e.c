...
static driver_t simcom_7600e_driver;

static void simcom_7600e_init( const thread_t * thread, const card_t *dev)
{
	SendCmd(thread, "AT");
	SendCmd(thread, "AT+CNMP=2");
...
static void simcom_7600e_sim( const thread_t *thread , const card_t *dev)
{
	SendCmd(thread, "AT+CPIN?");
	SendCmd(thread, "AT+CGSN");
}

static void simcom_7600e_imsi( const thread_t *thread , const card_t *dev)
{
	SendCmd(thread, "AT+CIMI");
	sleep(1);
}

static void simcom_7600e_signal ( const thread_t *thread, const card_t *dev )
{
	SendCmd(thread ,"AT+CSQ");
	sleep(1);

}
static void simcom_7600e_netmode( const thread_t *thread, const card_t *dev )
{
	SendCmd(thread, "AT+COPS?");
	sleep(1);
}

static void simcom_7600e_netscanmode( const thread_t *thread, const card_t *dev, const dial_t *cfg)
{
}

static void simcom_7600e_fix (const thread_t *thread, const card_t *dev , const dial_t *cfg)
{
	char cmd[256];
...
static int simcom_7600e_process( const thread_t *thread, const card_t *dev,  msg_t *msg)
{
	char buf[256];
	char *pos = NULL;
	int nbytes = 0;
	...
	while(1){
		...
		*pos = '\0';
		...
		/*msg in*/
		if( (pos = strstr(buf , "V4 DOWN")) != NULL){
...
static void simcom_7600e_reset( const thread_t *thread , const card_t *dev)
{
...
static void simcom_7600e_stop( const thread_t *thread , const card_t *dev)
{
	SendCmd(thread, "at$qcrmcall=0,1");
	sleep(3); //wait for disconnect
}

static int simcom_7600e_get_one_msg( const thread_t *thread, char *msg , unsigned int len)
{
	return read( thread->at_fd, msg, len);
}

static void simcom_7600e_send_cmd( const thread_t *thread, char *cmd)
{
	SendCmd(thread , cmd);
}

driver_t * simcom_7600e_driver_init( void )
{
	static int simcom_7600e_inited = 0;

	if( simcom_7600e_inited > 0)
		return &simcom_7600e_driver;
	simcom_7600e_inited++;
	simcom_7600e_driver.Vid = 0x1e0e;
	simcom_7600e_driver.Pid = 0x9001;
	strcpy(simcom_7600e_driver.sProduct, "7600e");
	strcpy(simcom_7600e_driver.sVendor, "Simcom");
	simcom_7600e_driver.datapath = 4;
	simcom_7600e_driver.atpath = 3;
	simcom_7600e_driver.DialType = NDIS_DIAL_TYPE;
	simcom_7600e_driver.Capability = MBNET_4G_MODE_TDLTE | MBNET_4G_MODE_TELECOM_FDDLTE | MBNET_4G_MODE_UNICOM_FDDLTE | \
									MBNET_3G_MODE_TDSCDMA |MBNET_3G_MODE_CDMA2000|MBNET_3G_MODE_WCDMA;
	simcom_7600e_driver.init = simcom_7600e_init;
	simcom_7600e_driver.sim = simcom_7600e_sim;
	simcom_7600e_driver.imsi = simcom_7600e_imsi;
	simcom_7600e_driver.signal = simcom_7600e_signal;
	simcom_7600e_driver.netmode = simcom_7600e_netmode;
	simcom_7600e_driver.netscanmode = simcom_7600e_netscanmode;
	simcom_7600e_driver.fix = simcom_7600e_fix;
	simcom_7600e_driver.process = simcom_7600e_process;
	simcom_7600e_driver.stop = simcom_7600e_stop;
	simcom_7600e_driver.reset = simcom_7600e_reset;
	simcom_7600e_driver.send_cmd = simcom_7600e_send_cmd;
	simcom_7600e_driver.get_one_msg = simcom_7600e_get_one_msg;
	simcom_7600e_driver.data =NULL;
	return &simcom_7600e_driver;
}
