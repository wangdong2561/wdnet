...
static driver_t nodecom_nl660_driver;

static void nodecom_nl660_init( const thread_t * thread, const card_t *dev)
{
	...
	/*link status report*/
	SendCmd(thread, "AT+PKTSTAQRY=1");
}

static void nodecom_nl660_sim( const thread_t *thread , const card_t *dev)
{
...
static void nodecom_nl660_imsi( const thread_t *thread , const card_t *dev)
{
...
static void nodecom_nl660_signal ( const thread_t *thread, const card_t *dev )
{
	SendCmd(thread ,"AT+CSQ");
	sleep(1);

}
static void nodecom_nl660_netmode( const thread_t *thread, const card_t *dev )
{
	SendCmd(thread, "AT+PSRAT");
	sleep(1);
}

static void nodecom_nl660_netscanmode( const thread_t *thread, const card_t *dev, const dial_t *cfg)
{
...
static void nodecom_nl660_fix (const thread_t *thread, const card_t *dev , const dial_t *cfg)
{
	char cmd[256];
...
static int nodecom_nl660_process( const thread_t *thread, const card_t *dev,  msg_t *msg)
{
	char buf[256];
	char *pos = NULL;
	int nbytes = 0;
...
static void nodecom_nl660_reset( const thread_t *thread , const card_t *dev)
{
...
static void nodecom_nl660_stop( const thread_t *thread , const card_t *dev)
{
	if(dev->CurrentNetMode == MBNET_3G_MODE_CDMA2000){
...
static int nodecom_nl660_get_one_msg( const thread_t *thread, char *msg , unsigned int len)
{
	return read( thread->at_fd, msg, len);
}

static void nodecom_nl660_send_cmd( const thread_t *thread, char *cmd)
{
	SendCmd(thread , cmd);
}

driver_t * nodecom_nl660_driver_init( void )
{
	static int nodecom_nl660_inited = 0;

	if( nodecom_nl660_inited > 0)
		return &nodecom_nl660_driver;
	nodecom_nl660_inited++;
	nodecom_nl660_driver.Vid = 0x1508;
	nodecom_nl660_driver.Pid = 0x1001;
	strcpy(nodecom_nl660_driver.sProduct, "nl660");
	strcpy(nodecom_nl660_driver.sVendor, "nodecom");
	nodecom_nl660_driver.datapath = 4;
	nodecom_nl660_driver.atpath = 3;
	nodecom_nl660_driver.DialType = NDIS_DIAL_TYPE;
	nodecom_nl660_driver.Capability = MBNET_4G_MODE_TDLTE | MBNET_4G_MODE_TELECOM_FDDLTE | MBNET_4G_MODE_UNICOM_FDDLTE | \
									MBNET_3G_MODE_TDSCDMA |MBNET_3G_MODE_CDMA2000|MBNET_3G_MODE_WCDMA;
	nodecom_nl660_driver.init = nodecom_nl660_init;
	nodecom_nl660_driver.sim = nodecom_nl660_sim;
	nodecom_nl660_driver.imsi = nodecom_nl660_imsi;
	nodecom_nl660_driver.signal = nodecom_nl660_signal;
	nodecom_nl660_driver.netmode = nodecom_nl660_netmode;
	nodecom_nl660_driver.netscanmode = nodecom_nl660_netscanmode;
	nodecom_nl660_driver.fix = nodecom_nl660_fix;
	nodecom_nl660_driver.process = nodecom_nl660_process;
	nodecom_nl660_driver.stop = nodecom_nl660_stop;
	nodecom_nl660_driver.reset = nodecom_nl660_reset;
	nodecom_nl660_driver.send_cmd = nodecom_nl660_send_cmd;
	nodecom_nl660_driver.get_one_msg = nodecom_nl660_get_one_msg;
	nodecom_nl660_driver.data =NULL;
	return &nodecom_nl660_driver;
}
