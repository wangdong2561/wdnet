...
static driver_t quectel_ec20v2_driver;

static void quectel_ec20v2_init( const thread_t * thread, const card_t *dev)
{
	SendCmd(thread, "AT");
...
static void quectel_ec20v2_sim( const thread_t *thread , const card_t *dev)
{
	SendCmd(thread, "AT+CPIN?");
	SendCmd(thread, "AT+GSN");
}

static void quectel_ec20v2_imsi( const thread_t *thread , const card_t *dev)
{
	SendCmd(thread, "AT+CIMI");
}

static void quectel_ec20v2_signal ( const thread_t *thread, const card_t *dev )
{
	SendCmd(thread ,"AT+CSQ");

}
static void quectel_ec20v2_netmode( const thread_t *thread, const card_t *dev )
{
	SendCmd(thread, "AT+COPS?");
}

static void quectel_ec20v2_netscanmode( const thread_t *thread, const card_t *dev, const dial_t *cfg)
{
...
static void quectel_ec20v2_fix (const thread_t *thread, const card_t *dev , const dial_t *cfg)
{
	char cmd[256];
	...
	if(quectel_ec20v2_driver.DialType != QMANAGE_DIAL_TYPE){
...
static int quectel_ec20v2_process( const thread_t *thread, const card_t *dev,  msg_t *msg)
{
	char buf[256];	
	char *pos = NULL;
	int nbytes = 0;
...
static void quectel_ec20v2_reset( const thread_t *thread , const card_t *dev)
{
...
static void quectel_ec20v2_stop( const thread_t *thread , const card_t *dev)
{
	if(quectel_ec20v2_driver.DialType != QMANAGE_DIAL_TYPE){
		if(dev->CurrentNetMode == MBNET_3G_MODE_CDMA2000){
...
static int quectel_ec20v2_get_one_msg( const thread_t *thread , char *msg , unsigned int len)
{
	return read( thread->at_fd, msg, len);
}

static void quectel_ec20v2_send_cmd( const thread_t *thread , char *cmd)
{
	SendCmd(thread , cmd);
}

driver_t * quectel_ec20v2_driver_init( void )
{
	static int quectel_ec20v2_inited = 0;

	if( quectel_ec20v2_inited > 0)
		return &quectel_ec20v2_driver;
	quectel_ec20v2_inited++;
	quectel_ec20v2_driver.Vid = 0x2c7c;
	quectel_ec20v2_driver.Pid = 0x0125;
	strcpy(quectel_ec20v2_driver.sProduct, "quectel_ec20v2");
	strcpy(quectel_ec20v2_driver.sVendor, "Quectel");
	quectel_ec20v2_driver.datapath = 3;
	quectel_ec20v2_driver.atpath = 2;
	quectel_ec20v2_driver.DialType = QMANAGE_DIAL_TYPE;
	quectel_ec20v2_driver.Capability = MBNET_4G_MODE_TDLTE | MBNET_4G_MODE_TELECOM_FDDLTE | MBNET_4G_MODE_UNICOM_FDDLTE | \
									MBNET_3G_MODE_TDSCDMA |MBNET_3G_MODE_CDMA2000|MBNET_3G_MODE_WCDMA;
	quectel_ec20v2_driver.init = quectel_ec20v2_init;
	quectel_ec20v2_driver.sim = quectel_ec20v2_sim;
	quectel_ec20v2_driver.imsi = quectel_ec20v2_imsi;
	quectel_ec20v2_driver.signal = quectel_ec20v2_signal;
	quectel_ec20v2_driver.netmode = quectel_ec20v2_netmode;
	quectel_ec20v2_driver.netscanmode = quectel_ec20v2_netscanmode;
	quectel_ec20v2_driver.fix = quectel_ec20v2_fix;
	quectel_ec20v2_driver.process = quectel_ec20v2_process;
	quectel_ec20v2_driver.stop = quectel_ec20v2_stop;
	quectel_ec20v2_driver.reset = quectel_ec20v2_reset;
	quectel_ec20v2_driver.send_cmd = quectel_ec20v2_send_cmd;
	quectel_ec20v2_driver.get_one_msg = quectel_ec20v2_get_one_msg;
	quectel_ec20v2_driver.data =NULL;
	return &quectel_ec20v2_driver;
}
