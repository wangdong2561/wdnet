...
static driver_t longsung_u8300c_driver;

static void longsung_u8300c_init( const thread_t * thread, const card_t *dev)
{
...
static void longsung_u8300c_sim( const thread_t *thread , const card_t *dev)
{
...
static void longsung_u8300c_imsi( const thread_t *thread , const card_t *dev)
{
...
static void longsung_u8300c_signal ( const thread_t *thread, const card_t *dev )
{
	SendCmd(thread ,"AT+CSQ");
	sleep(1);

}
static void longsung_u8300c_netmode( const thread_t *thread, const card_t *dev )
{
	SendCmd(thread, "AT+PSRAT");
	sleep(1);
}

static void longsung_u8300c_netscanmode( const thread_t *thread, const card_t *dev, const dial_t *cfg)
{
...
static void longsung_u8300c_fix (const thread_t *thread, const card_t *dev , const dial_t *cfg)
{
	char cmd[256];
...
static int longsung_u8300c_process( const thread_t *thread, const card_t *dev,  msg_t *msg)
{
	char buf[256];
	char *pos = NULL;
	int nbytes = 0;
...
static void longsung_u8300c_reset( const thread_t *thread , const card_t *dev)
{
...
static void longsung_u8300c_stop( const thread_t *thread , const card_t *dev)
{
	if(dev->CurrentNetMode == MBNET_3G_MODE_CDMA2000){
...
static int longsung_u8300c_get_one_msg( const thread_t *thread, char *msg , unsigned int len)
{
	return read( thread->at_fd, msg, len);
}

static void longsung_u8300c_send_cmd( const thread_t *thread, char *cmd)
{
	SendCmd(thread , cmd);
}

driver_t * longsung_u8300c_driver_init( void )
{
	static int longsung_u8300c_inited = 0;

	if( longsung_u8300c_inited > 0)
		return &longsung_u8300c_driver;
	longsung_u8300c_inited++;
	longsung_u8300c_driver.Vid = 0x1c9e;
	longsung_u8300c_driver.Pid = 0x9b05;
	strcpy(longsung_u8300c_driver.sProduct, "u8300c");
	strcpy(longsung_u8300c_driver.sVendor, "longsung");
	longsung_u8300c_driver.datapath = 4;
	longsung_u8300c_driver.atpath = 3;
	longsung_u8300c_driver.DialType = NDIS_DIAL_TYPE;
	longsung_u8300c_driver.Capability = MBNET_4G_MODE_TDLTE | MBNET_4G_MODE_TELECOM_FDDLTE | MBNET_4G_MODE_UNICOM_FDDLTE | \
									MBNET_3G_MODE_TDSCDMA |MBNET_3G_MODE_CDMA2000|MBNET_3G_MODE_WCDMA;
	longsung_u8300c_driver.init = longsung_u8300c_init;
	longsung_u8300c_driver.sim = longsung_u8300c_sim;
	longsung_u8300c_driver.imsi = longsung_u8300c_imsi;
	longsung_u8300c_driver.signal = longsung_u8300c_signal;
	longsung_u8300c_driver.netmode = longsung_u8300c_netmode;
	longsung_u8300c_driver.netscanmode = longsung_u8300c_netscanmode;
	longsung_u8300c_driver.fix = longsung_u8300c_fix;
	longsung_u8300c_driver.process = longsung_u8300c_process;
	longsung_u8300c_driver.stop = longsung_u8300c_stop;
	longsung_u8300c_driver.reset = longsung_u8300c_reset;
	longsung_u8300c_driver.send_cmd = longsung_u8300c_send_cmd;
	longsung_u8300c_driver.get_one_msg = longsung_u8300c_get_one_msg;
	longsung_u8300c_driver.data =NULL;
	return &longsung_u8300c_driver;
}
